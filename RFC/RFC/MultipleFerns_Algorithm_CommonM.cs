...
namespace RFC
{
    class MultipleFerns_Algorithm_CommonM
    {
        ...
        public void Calculation_FernsImage_Common(string imageName, double[,] sampleG, int featureNumber, int fernsNumber, int numClusters, int sizeFeatures, int maxIterations, double percision, out Bitmap BinaryNumber2, out int Iterations, out double[][,] allMean,out double[][,] allinitialMu, out int[][] allGCluster, out int[,] fea, out int[,] r, out string result, out int[] cluster_, out double[,] s, out double[] mmm)
        {
        ...
        public void ferns_EMstep(double[,] sampleG, int featureNumber, int fernsNumber, int sizeFeatures, int numClusters, int maxIterations, double percision, out int Iterations, out double[] mmm, out double[][,] allMean, out double[][,] allinitialMu, out int[,] fea, out int[,] r1, out int[][] allGCluster, out string result, out int[] cluster_, out double[,] s)
        {
            ...
            do
            {
                ...
                do
                {
                    if (maxIterations > 0)
                    {
                        Pxgi_ck(sampleG1, mu_Mstep, var_Mstep, numClusters, sizeFeatures, out pxgi_ck);
                        PxgCk(sampleG1.GetLength(0), numClusters, sizeFeatures, pxgi_ck, out pxg_ck);//Pxg_ck
                        P_Xg(sampleG1.GetLength(0), numClusters, pck, pxg_ck, out pxg);//計算Pxg
            ...
            commonMember(fernsNumber, numClusters, fernpcknumber, allGCluster, out com);
            Compared_Ferns_commonMember_(com, fernsNumber, numClusters, fernpcknumber, out _r1); //0924新增!!!!!!!!!! 比對ferns!!!

           // FernIJ(feature_Fmeans); //feature_Fmeans=分群完的結果中各群的特徵值之平均值
           // Compared_Ferns(fernij, fernsNumber, numClusters, fernpcknumber, out _r1); //利用分群結果中各群的平均值相減
        ...
        private void FernIJ(double[, ,] feature_Fmeans) //feature_Fmeans = 計算分群完的結果中各群個特徵值之平均值
        {
        ...
        //--------------------------------------------
        // 計算分群器間共同的members數量
        // com[ferns-1,群別數目,群別數目], max =com[ferns-1,群別數目,群別數目]中最大的值
        //--------------------------------------------
        public void commonMember(int fernsNumber, int numClusters, double[,] fernpcknumber, int[][] allGCluster , out int[, ,] com)
        {
            
            int[,][] comCluster = new int[fernsNumber, numClusters][];
            int[,] memberNum = new int[numClusters, numClusters];
            com = new int[fernsNumber - 1, numClusters, numClusters];
            double max = -1;

            for (int f = 0; f < fernsNumber; f++)
            {
                for (int n = 0; n < numClusters; n++)
                {
                    comCluster[f, n] = new int[Convert.ToInt32(fernpcknumber[f, n])];
                }
            }
            double[,] a = new double[fernpcknumber.GetLength(0), fernpcknumber.GetLength(1)];
            for (int i = 0; i < fernpcknumber.GetLength(0); i++)
            {
                for (int j = 0; j < fernpcknumber.GetLength(1); j++)
                {
                    a[i, j] = fernpcknumber[i, j];
                }
            }
            for (int i = 0; i < allGCluster[0].Length; i++)
            {
                for (int f = 0; f < fernsNumber; f++)
                {
                    for (int n = 0; n < numClusters; n++)
                    {
                        ...
                        if (allGCluster[f][i] == n)
                        {
                            comCluster[f, n][Convert.ToInt32(fernpcknumber[f, n]) - Convert.ToInt32(a[f, n])] = i;
                            a[f, n]--;
                            break;
                        }
                    }
            ...
            {
                for (int f = i + 1; f < fernsNumber; f++)
                {
                    for (int n = 0; n < numClusters; n++)
                    {
                        for (int m = 0; m < numClusters; m++)
                        {
                            for (int k = 0; k < comCluster[i, n].Length; k++)
                            {
                                for (int l = 0; l < comCluster[f, m].Length; l++)
                                {
                                    if (comCluster[i, n][k] == comCluster[f, m][l])
                                    {
                                        com[i, n, m] += 1;
                                        if (com[i, n, m] > max)
                                        {
