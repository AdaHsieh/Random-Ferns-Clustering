...
namespace RFC
{
    class MultipleFerns_Algorithm_NumberM
    {
        ...
        public void ferns_EMstep(double[,] sampleG, int featureNumber, int fernsNumber, int sizeFeatures, int numClusters, int maxIterations, double percision, out int Iterations, out double[] mmm, out double[][,] allMean, out double[][,] allinitialMu, out int[,] fea, out int[,] r1, out int[][] allGCluster, out string result, out int[] cluster_, out double[,] s)
        {
            ...
            do
            {
                ...
                Newmean(sampleG, numClusters, sizeFeatures, out initialMu);
            ...
            Compared_Ferns_memberNumber(fernij, fernsNumber, numClusters, fernpcknumber, out _r1);
        ...
        private void Compared_Ferns_memberNumber(double[,] fernij, int fernsNumber, int numClusters, double[,] fernpcknumber, out int[,] _r1) //ferns size, sample size, cluster size
        {
            newFern = new double[numClusters]; //存取newFerns(用來持續修正)
            //第一次比對(尚未開始加入修正的值)
            int a = 0;
                ...
                while (b < fernsNumber)
                {
                    if (b == 1)
                    {
                        ...
                        for (int i = 0; i < numClusters; i++)
                        {
                            for (int j = 0; j < numClusters; j++)
                            {
                                a_b[i, j] = Math.Abs(fernpcknumber[a, i] - fernpcknumber[b, j]);
                            }
                        }
                        //--------------//
                        //利用指派問題 - 匈牙利法 得到最佳的指派
                        ...
                        for (int i = 0; i < numClusters; i++)
                        {
                            ...
                            {
                                newFern[i] = (fernpcknumber[a, _r1[a, i]] + fernpcknumber[b, _r1[b, i]]) / 2.0;
                            }
                        }
                    ...
                    else
                    {
                        for (int i = 0; i < numClusters; i++) //計算newFern與其他fern之值
                        {
                            for (int j = 0; j < numClusters; j++)
                            {
                                a_b[i, j] = Math.Abs(newFern[i] - fernpcknumber[b, j]);
                            }
                        }
                        ZMatrix m = new ZMatrix(numClusters, numClusters, a_b);
                        m.Calculation();
                        ...
                        {
                            newNumber[i] = newNumber[i] + fernpcknumber[b, _r1[b, i]];
                            newFern[i] = ((newNumber[i]) + fernpcknumber[b, _r1[b, i]]) / (1 + b);
                        }
